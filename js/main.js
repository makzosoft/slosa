...
// Contact form sends straight to WhatsApp, formatted so the secretariat can read it at a glance
const contactForm = document.getElementById('contactForm');
if (contactForm) {
  const submitBtn = contactForm.querySelector('button[type="submit"]');
  const whatsappNumber = '2348023382628'; // first number provided by the secretariat
  ...
  contactForm.addEventListener('submit', (e) => {
    ...
    const set = contactForm.querySelector('#cf-set').value.trim();
    const message = contactForm.querySelector('#cf-message').value.trim();

    const lines = [
      '*New message from the SLOSA website*',
      '',
      `*Name:* ${name}`,
      `*Email:* ${email}`,
      `*Set or graduating year:* ${set || 'Not provided'}`,
      '',
      '*Message:*',
      message
    ];
